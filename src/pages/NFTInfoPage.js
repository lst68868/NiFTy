...
function NFTInfoPage() {
  ...
  async function getNFT() {
    try {
      ...
      data.forEach((nft) => {
        //<-- We need a route that responds with a single nft so that we don't have to loop through every nft to find the one we need
        if (nft.id == id) {
  ...
  return (
    <div className="nft-info-page">
      ...
      <div className="info-container">
        ...
        <p className="owner">
          Owned by: <span className="light-blue">{nftOwner}</span> | Created by:{" "}
          <span className="light-blue">{nftCreator}</span>
        </p>
